...
def get_rp_binary():
    """
    Returns (binary_path, download_url) tuple for the current platform.
    ...
    if system == "Linux":
        fname = "rp-lin-x64"
        rp_path = Path("~/.local/bin/").expanduser().resolve() / fname
    elif system == "Darwin":
        fname = "rp-osx-x64"
        rp_path = Path("~/.local/bin/").expanduser().resolve() / fname
    elif system == "Windows":
        fname = "rp-win-x64.exe"
        # Use %LOCALAPPDATA%\rp\ — always writable, no UAC required
        local_app_data = Path.home() / "AppData" / "Local" / "rp"
        rp_path = local_app_data / fname
    else:
        raise RuntimeError(f"Unsupported platform: {system}")

    download_url = (
        f"https://github.com/0vercl0k/rp/releases/download/{RRP_VERSION}/{fname}"
    )
    return rp_path, download_url


def download_rp(rp_path, download_url):
    """
    Downloads rp++ to rp_path using urllib (no external tools required).
    Sets executable bit on Linux/macOS.
    """
    rp_path.parent.mkdir(parents=True, exist_ok=True)

    print(f"[bright_yellow][*][/bright_yellow] rp++ not found, downloading {RRP_VERSION}...")
    print(f"[bright_green][+][/bright_green] URL: {download_url}")

    try:
        urllib.request.urlretrieve(download_url, rp_path)
    except Exception as e:
        print(f"[bright_red][!][/bright_red] Download failed: {e}")
        print(
            f"[bright_red][!][/bright_red] Download manually from {download_url} "
            f"to {rp_path}, or add -s|--skip-rp to skip rp++."
        )
        return False

    # chmod not needed (or meaningful) on Windows
    if platform.system() != "Windows":
        rp_path.chmod(mode=0o755)

    print(f"[bright_green][+][/bright_green] rp++ saved to {rp_path}")
    return True
...
def add_missing_gadgets(ropper_addresses: set, in_file, outfile, bad_bytes, base_address=None):
    ...
    This function runs rp++ and appends any addresses not already found by ropper.
    Supports Linux, macOS, and Windows.
    """
    try:
        rp, download_url = get_rp_binary()
    except RuntimeError as e:
        print(f"[bright_red][!][/bright_red] {e} — skipping rp++")
        return

    if not rp.exists():
        if not download_rp(rp, download_url):
            return

    # Build command as a list to avoid shell quoting issues on all platforms
    # and to handle paths with spaces correctly (common on Windows).
    ...
    # capture_output=True avoids TemporaryFile stdout redirection, which has
    # Windows file handle inheritance issues with subprocess.
...
def print_useful_regex(outfile, arch):
    ...
    # Note: awk/sort/cut are Unix tools. On Windows, open the output file
    # in a text editor or use: findstr /R "<pattern>" <outfile>
